...
export function createOrderRoute(_database: DatabasePlugin) {
	return new Elysia().use(authGuard("customer")).post(
		"/",
		({ status, userId }) => {
			const result = service.createPaypalOrder("10.00", "USD");
			return result.match(
