...
export function DealProductRow({
  ...
  const priceLabel = useMemo(() => formatEur(item.unitPriceCents), [item.unitPriceCents]);
  const originalLabel = useMemo(
    () => (typeof item.originalUnitPriceCents === 'number' ? formatEur(item.originalUnitPriceCents) : null),
    ...
    <View style={styles.row}>
      ...
      <View style={styles.right}>
        <View style={styles.priceRow}>
          ...
          {originalLabel ? <Text style={styles.originalPrice}>{originalLabel}</Text> : null}
        </View>
        <View style={styles.buttonWrap}>
          <PrimaryButton title={actionLabel} onPress={() => onPressAction(item)} />
        </View>
      </View>
    </View>
  );
